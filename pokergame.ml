...
module type Poker = sig
  ...
  val parse_move : string -> pokerhand option -> move
end
...
let convert_rank_to_phand hand =
  match hand with
  | a::[] -> HighCard a
  | a::b::[] -> Pair a
  ...
  let parse_move move ph =
    ...
    if num_words = 0 then raise InvalidMove
      ...
      if num_words = 1 && move_type = "bs"then
        match ph with
        | None -> raise InvalidBS
        | Some p -> BS p
        ...
        let prev =
          ...
          | Some p -> p
        in
        if compare_phands raised prev = 1 then
...
module GameRound : Round = struct
 ...
 let rec human_turn h ph =
    ...
    let () =
      ...
      | None -> ()
      | Some p ->
        print_endline ("The previous call was: "^(string_of_pokerhand p))
    in
